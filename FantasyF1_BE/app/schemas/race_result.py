...
class RaceResultBase(BaseModel):
    ...
    position: int = Field(
        ..., ge=0, le=20, description="Final finishing position (1-20, 0 for DNF)"
    )
    grid_position: int | None = Field(
        None, ge=1, le=20, description="Starting position on the grid"
    )
    ...
    time_diff_str: str | None = Field(
        None, max_length=20, description="String representation of time difference"
    )
...
class RaceResultResponse(RaceResultBase):
    ...
    position_change: int | None = Field(
        None, description="Positions gained (+) or lost (-) from grid to finish"
    )
